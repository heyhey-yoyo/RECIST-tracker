...
界面采用 YDchenTools 工具站统一视觉语言：主体使用 `ydchen-portfolio` 的米白、浅灰与赤陶色配色、衬线标题和扁平化内容卡片；页眉保留半透明白色背景、`YDchen` 衬线字标、浅灰 `Tools` 字样、竖向分隔线及紧凑的双行工具标题。主功能导航位于页眉下方，并针对桌面和移动端分别优化。
